...
bool GLContext::InitEGLSurface() {
  ...
  if (!num_configs) {
    Logger::writeError("Unable to retrieve EGL config");
    return false;
  }
...
bool GLContext::InitEGLContext() {
  ...
  if (eglMakeCurrent(display_, surface_, surface_, context_) == EGL_FALSE) {
    Logger::writeError("Unable to eglMakeCurrent");
    return false;
  }
...
EGLint GLContext::Resume(ANativeWindow* window) {
  ...
  if (screen_width_ != original_widhth || screen_height_ != original_height) {
    // Screen resized
      Logger::writeInfo("Screen resized");
  }
  ...
  EGLint err = eglGetError();
  Logger::writeError("Unable to eglMakeCurrent %d", err);

  if (err == EGL_CONTEXT_LOST) {
    // Recreate context
    Logger::writeInfo("Re-creating egl context");
    InitEGLContext();
  } else {
    // Recreate surface
    Terminate();
